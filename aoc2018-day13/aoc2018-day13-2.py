...
class Cart():
  ...
  def act(self):
    ...
    # Test for crash
    for c in carts:
      if c.uid != self.uid and c.x == self.x and c.y == self.y and c.active:
...
def tick():
  ...
  [c.act() for c in carts if c.active]

  actives = [c for c in carts if c.active]
  if len(actives) == 1:
    print('Final cart position -- x: ' + str(actives[0].x) + ', y: ' + str(actives[0].y))
    exit()
