...
cart_grid = [[{} for _ in range(h)] for _ in range(w)]
new_cart_grid = [[{} for _ in range(h)] for _ in range(w)]
...
class Cart():
  ...
  def act(self):
    global cart_grid
    global new_cart_grid
    cart_grid[self.x][self.y] = {}

    self.x += deltas[self.d][0]
    self.y += deltas[self.d][1]

    # test for crash
    if new_cart_grid[self.x][self.y] != {} or cart_grid[self.x][self.y] != {}:
      print('Crash!!! ' + str(self.x) + ', ' + str(self.y))
      exit()
      
    new_cart_grid[self.x][self.y] = self
...
def setup():
  ...
  for i in range(0, w):
    for j in range(0, h):
      if data[j][i] in ["<", ">", "^", "v"]:
        cart_grid[i][j] = Cart(i, j, dirs[data[j][i]])
